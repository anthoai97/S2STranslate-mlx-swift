...
- Current checkpoint: the first Experiment Session skeleton, model artifact preparation boundary, reference trace harness, simulator-friendly fixture audio input slice, downloadable French file input slice, deterministic Mimi streaming encode/decode boundaries, deterministic Hibiki inference boundary, and buffered playback sink are implemented. The app has a Moshi-inspired proof-of-concept control surface with prepare, input selection, start, stop, failure-demo, and new-session flows, plus observations for artifact progress, file/fixture PCM chunk metrics, Mimi token-frame cadence, Hibiki text/audio-token event flow, decoded PCM chunk cadence, and playback delivery. It still does not perform real model loading, live microphone capture, MLX-backed Mimi encoding/decoding, MLX-backed Hibiki inference, audible device playback, or real translation.
