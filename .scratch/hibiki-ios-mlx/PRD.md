...
- Current checkpoint: the first Experiment Session skeleton, real Hugging Face artifact download/cache path, reference trace harness, simulator-friendly fixture audio input slice, downloadable French file input slice, deterministic Mimi streaming encode/decode boundaries, deterministic Hibiki inference boundary, and buffered playback sink are implemented. The app has a Moshi-inspired proof-of-concept control surface with prepare, artifact download progress, input selection, start, stop, failure-demo, and new-session flows, plus observations for artifact progress, file/fixture PCM chunk metrics, Mimi token-frame cadence, Hibiki text/audio-token event flow, decoded PCM chunk cadence, and playback delivery. It still does not load model weights into MLX, perform live microphone capture, run MLX-backed Mimi encoding/decoding, run MLX-backed Hibiki inference, produce audible device playback, or perform real translation.
